...
function setup() 
{
  ...
  star3 = createSprite(500,267,20,20);
  star3.addImage(starImg);
  star3.scale = 0.02;

  starDisplay = createSprite(70,35,30,30);
  starDisplay.scale = 0.2;
  starDisplay.addAnimation("empty",emptystar);
  starDisplay.addAnimation("one",Star1);
  starDisplay.addAnimation("two",Star2);
...
function airLeftBlow(){
  Matter.Body.applyForce(fruit,{x:0,y:0},{x:0.03,y:0});
  air.play();
}

function Collide(body,sprite,s){
  if(body != null){
    var d = dist(body.position.x,body.position.y,sprite.position.x,sprite.position.y);
    if(d<=s){
      return true;
